using OhMyREPL, Flux, MLDataUtils, CSV, Statistics, LinearAlgebra, DataStructures, Distances, CuArrays, BSON, StatsBase, JLD, CUDAnative
...
# for i in 1:10
    ...
    #     @show loss(cu(getobs(imgs[:,:,:,3131995ixs])), cu(getobs(labels[ixs])))
...
path = string("mac_param_experiments10_mg_maze/",folder_name)
mkdir(path)
